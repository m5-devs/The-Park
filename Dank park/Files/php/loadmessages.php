...
//Delete old messages
mysql_query("DELETE FROM messages WHERE timestamp < (NOW() - INTERVAL 24 HOUR)");
...
while ($message = mysql_fetch_assoc($messages)) {
	if ($message['userid']==$sesuser['id']) {
		...
		$lastid = $message['id']-1;
		$lastmessage = mysql_fetch_assoc(mysql_query("SELECT * FROM messages WHERE id = '$lastid'"));
		...
		//Get info for last message in the loop
		$lastid = $message['id']-1;
		$lastmessage = mysql_fetch_assoc(mysql_query("SELECT * FROM messages WHERE id = '$lastid' ORDER BY timestamp"));
		...
		$nextid = $message['id']+1;
		$nextmessage = mysql_fetch_assoc(mysql_query("SELECT * FROM messages WHERE id = $nextid"));
