...
if (isset($colorchoice)) {
	mysql_query("UPDATE users SET color = '$colorchoice' WHERE id = '$userid'");
}
if (isset($censor)) {
	if ($censor == 'false') {
		$censorval = '0';
	...
	mysql_query("UPDATE users SET filter = '$censorval' WHERE id = '$userid'");
	echo $censorval;
}
if (isset($password)) {
		...
		if ($newpass == $confirmpass) {
			if ($newpass != $oldpass) {
				if (strlen($newpass) > 7) {
					...
					if (mysql_query("UPDATE users SET password = '$hash', salt = '$salt' WHERE id = '$userid'")) {
						echo 'changed';
					} else {
...
if (isset($theme)) {
	...
	if (!mysql_query("UPDATE users SET theme = '$themeval' WHERE id = '$userid'")) {
		echo 'fail';
	}
}
...
if (strlen($avatar) > 0) { mysql_query("UPDATE users SET avatar = '$avatar' WHERE id = '$userid'"); }
if (strlen($cover) > 0) { mysql_query("UPDATE users SET cover = '$cover' WHERE id = '$userid'"); }
