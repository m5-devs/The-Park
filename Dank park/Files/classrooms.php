		...
		<div class="card" id="group-classes">
			<ul id="classlist" style="margin: 0;">
				...
				if ($teacher) {//Teacher account
					//Get ids of classes
					$classquery = mysql_query("SELECT * FROM classes WHERE teacherid = '$userid' ORDER BY period, ab ASC");
				} else {//Student account
					
					//Get ids and periods of classes
					$classquery = mysql_query("SELECT classid FROM students WHERE studentid = '$userid'");
					$classids = Array();
					while ($row = mysql_fetch_array($classquery, MYSQL_ASSOC)){ $classids[] = "'".$row['classid']."'"; }
					$classids = implode(',', $classids);
					//Second sql query run because students table doesn't have sufficient information
					if (mysql_num_rows($classquery) != 0) {$classquery = mysql_query("SELECT * FROM classes WHERE id in ($classids) ORDER BY period, ab ASC"); }
				}
				//List classes
		  ...
		  <?php } else {
					while ($classinfo = mysql_fetch_array($classquery)) {
						//Get information about teacher
						$teacherinfo = getInfo((int) $classinfo['teacherid']);
						$classid = $classinfo['id'];
						if ($teacher) {
							//Get class code
							$classcodearray = mysql_fetch_array(mysql_query("SELECT code FROM classcodes WHERE classid = '$classid'"));
							$classcode = $classcodearray['code'];
						}
					?>
