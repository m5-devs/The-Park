...
$escapecomment = mysql_real_escape_string($comment);
$postinfo = mysql_fetch_assoc(mysql_query("SELECT * FROM posts WHERE id = '$postid'")); //Get info about post
...
if (strlen($comment) == 0) {
	...
	if (mysql_query("INSERT INTO comments (post_id, userid, comment) VALUES ('$postid','$userid','$escapecomment')")) {
		echo '
		...
		if ($postuserid != $_SESSION['id']) {
			mysql_query("INSERT INTO notifications (userid, type, user_from, id_from) VALUES ('$postuserid','comment','$userid','$postid')");
		}
	} else {
		echo 'fail';
	}
