...
$checkusername = mysql_num_rows(mysql_query("SELECT * FROM users WHERE username = '$username'"));
$checkemail = mysql_num_rows(mysql_query("SELECT * FROM users WHERE email = '$email'"));
											...
											if (strlen($password) != 0) { //Check empty field
												if (strlen($password) > 7) { //Password has to be 8+ chars
													//if (!strpos($password, $username)) {
														if (mysql_query($insertsql)) { //Create user
															$id = getInfo($username, 'id'); //Get id of user
															if (mysql_query("INSERT INTO verify (id, code) VALUES ('$id','$code')")) { //Create verification row
