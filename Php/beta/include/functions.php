...
function getFriends($userid) {
	$result = $db->select("SELECT user_from FROM friends WHERE user_to = '$userid' UNION SELECT user_to FROM friends WHERE user_from = '$userid'");
	global $all_friends;
	$all_friends = array();

	while ($friendrow = mysqli_fetch_assoc($result)) {
...
function getInfo($identifier, $column='all') {
	global $db;
	
	if (is_numeric($identifier)) {
 		$infoquery = $db->select("SELECT * FROM users WHERE id = '$identifier'");
	} else {
 		$infoquery = $db->select("SELECT * FROM users WHERE username = '$identifier'");
	}
	$infoquery = $infoquery[0];
	if (count($infoquery) == 0) { return false; } else {
		if ($column == 'all') {
			global $user_info;
...
function checkFriends($name1, $name2) {
	$number = count($db->select("SELECT id FROM friends WHERE user_to = '$name1' AND user_from = '$name2' OR user_to = '$name2' AND user_from = '$name1'"));
	if ($number == 1) {
		return true;
	} else {
		return false;
...
// 	$accepted_array = $db->select("SELECT accepted FROM friends WHERE user_from = '$name1' AND user_to = '$name2' OR user_to = '$name2' AND user_from = '$name1'");
// 	var_dump($accepted_array);
...
function verifyPass($username, $password) {
	global $db;
    $userinfo = $db->select("SELECT password, salt FROM users WHERE username = '$username'");
	$hash = crypt($password, '$6$rounds=5000$'.$userinfo[0]['salt'].'$');
	
	if ($userinfo[0]['password']==$hash) {
		return true;
	} else {
		return false;
	}
...
function notify($userid, $type, $userfrom, $idfrom) {
	$db->select("INSERT INTO notifications (userid, type, user_from, id_from) VALUES ('$userid','$type','$userfrom','$idfrom')");
}
...
function generateCode($length = 6) {
	...
	if (count($db->select("SELECT * FROM classcodes WHERE code='$string'")) == 0) {
		return $string;
	} else {
		generateCode($length);
	}
