<?php
	...
	$classid = $_GET['id'];
	$userid = $sesuser['id'];
	$classinfo = $db->select("SELECT * FROM classes WHERE id='$classid'");
	$classinfo = $classinfo[0];
	$studentinfo = $db->select("SELECT * FROM students WHERE studentid='$userid' AND classid='$classid'");
	
	//Redirect student if not in class and user is not the teacher, if class doesn't exist, or if not signed in
	// if (count($studentinfoquery) == 0 and $classinfo['teacherid'] != $userid or !isset($_SESSION['id']) or count($classinfoquery) == 0) {
